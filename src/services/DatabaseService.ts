...
export class DatabaseService implements DatabaseMethods {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      this.run = promisify(this.db.run.bind(this.db)) as any;
  ...
  async saveUserProfile(userId: string, userProfile: UserProfile): Promise<UserProfile> {
    try {
  ...
  async getAllUserProfiles(limit: number = 100, offset: number = 0): Promise<Array<{
    userId: string;
    profile: UserProfile;
    createdAt: string;
    updatedAt: string;
  ...
  async getConversationHistory(userId: string, limit: number = 50, offset: number = 0): Promise<Array<{
    id: number;
    messageType: 'user' | 'assistant' | 'system';
    messageText: string;
  ...
  async recordInteraction(
    userId: string, 
    interactionType: 'chat' | 'intake' | 'recommendation', 
  ...
  async getInteractionStats(userId: string, days: number = 30): Promise<Array<{
  ...
  async updateRecommendationFeedback(
    ...
    try {
      await this.run(`
        ...
        WHERE id = ?
      `, [wasHelpful, feedback || null, recommendationId]);
    } catch (error) {
  ...
  async getRecommendationHistory(userId: string, limit: number = 20): Promise<Array<{
    id: number;
    type: string;
  ...
  async getUserAnalytics(userId: string): Promise<{
    ...
    try {
      ...
      return {
        ...
        totalInteractions: profile.interactionHistory?.totalInteractions || 0,
